...
class BaseComputation(Configurable, ABC):
    ...
    def __exit__(self, exc_type: None, exc_value: None, traceback: None) -> None:
        if exc_value and isinstance(exc_value, VMError):
            ...
            # print("COMPUTATION ERROR:\"{}\"".format(exc_value))
    ...
    def apply_computation(cls,
        ...
        with cls(state, message, transaction_context) as computation:
            ...
            for opcode in computation.code:
                ...
                # convert stack value from bytes to 0x
                convert_stack_value = []
                for i in range(len(computation._stack.values)):
                    if type(computation._stack.values[i]) == int:
                        x = hex(computation._stack.values[i])
                        convert_stack_value.append("\"" + str(x) + "\"")
                    else:
                        x = hex(int.from_bytes(computation._stack.values[i], byteorder='little'))
                        convert_stack_value.append("\"" + str(x) + "\"")

                print("{{\"pc\": {}, \"op\": {}, \"gas\": \"0x{}\", \"gasCost\": \"0x{}\","
                      " \"stack\": [{}], \"depth\": {}, \"opName\": \"{}\", \"error\": \"{}\" }}"
                      .format(max(0, computation.code.pc - 1),
                              opcode,
                              ('%x' % computation._gas_meter.gas_remaining),
                              ('%x' % opcode_fn.gas_cost),
                              (", ".join(convert_stack_value)),
                              computation.msg.depth,
                              opcode_fn.mnemonic,
                              computation._error))
